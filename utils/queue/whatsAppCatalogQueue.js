...
import { processWhatsAppCatalogWithApi } from "../whatsapp/processWhatsAppCatalogWithApi.js";

const adminPhone = process.env.ADMIN_PHONE;

// Class definition for the Queue
export class WhatsAppCatalogQueue {
	constructor() {
		this.queues = new Map();
	}
	...
	async processQueue(senderId) {
		...
		while (queue.messages.length > 0) {
			...
			try {
				// Process the message
				const log = await processWhatsAppCatalogWithApi(userMessage);
				console.log(log);
			} catch (error) {
				...
				const message = `🔔 *NOTIFICACION DE ERROR AL ADMIN en whatsAppCatalogQueue.js:*\nLead ${userMessage.name} Cel. ${userMessage.userPhone}.\nError: ${errorMessage}.`;
